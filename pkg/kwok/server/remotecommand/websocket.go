...
// createWebSocketStreams returns a streamContext containing the websocket connection and
// streams needed to perform an exec or an attach.
func createWebSocketStreams(req *http.Request, w http.ResponseWriter, opts *Options, idleTimeout time.Duration) (*streamContext, bool) {
	...
	if err != nil {
		err := fmt.Errorf("unable to upgrade websocket connection: %w", err)
		logger := log.FromContext(req.Context())
		logger.Error("open stream", err)
		return nil, false
	}
	...
	switch {
	case opts.Stdout:
		_, _ = streams[stdoutChannel].Write([]byte{})
	case opts.Stderr:
		_, _ = streams[stderrChannel].Write([]byte{})
	default:
		_, _ = streams[errorChannel].Write([]byte{})
	}

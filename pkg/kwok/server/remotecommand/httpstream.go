...
func NewOptions(req *http.Request) (*Options, error) {
	...
	if tty && stderr {
		...
		logger.Warn("Access to exec with tty and stderr is not supported, bypassing stderr")
		stderr = false
	}
...
// streamContext contains the connection and streams used when
...
func createStreams(req *http.Request, w http.ResponseWriter, opts *Options, supportedStreamProtocols []string, idleTimeout, streamCreationTimeout time.Duration) (*streamContext, bool) {
...
func createHTTPStreamStreams(req *http.Request, w http.ResponseWriter, opts *Options, supportedStreamProtocols []string, idleTimeout, streamCreationTimeout time.Duration) (*streamContext, bool) {
	...
	case "":
		logger := log.FromContext(req.Context())
		logger.Info("Client did not request protocol negotiation. Falling back", "protocol", remotecommandconsts.StreamProtocolV1Name)
		fallthrough
	...
	ctx, err := handler.waitForStreams(req.Context(), streamCh, expectedStreams, expired.C)
	if err != nil {
		logger := log.FromContext(req.Context())
		logger.Error("waitForStreams", err)
		return nil, false
	}
...
type protocolHandler interface {
	...
	waitForStreams(ctx context.Context, streams <-chan streamAndReply, expectedStreams int, expired <-chan time.Time) (*streamContext, error)
...
func (*v4ProtocolHandler) waitForStreams(ctx context.Context, streams <-chan streamAndReply, expectedStreams int, expired <-chan time.Time) (*streamContext, error) {
	sctx := &streamContext{}
	receivedStreams := 0
	...
	for {
		...
		case stream := <-streams:
			...
			case corev1.StreamTypeError:
				sctx.writeStatus = v4WriteStatusFunc(stream) // write json errors
				go waitStreamReply(stream.replySent, replyChan, stop)
			case corev1.StreamTypeStdin:
				sctx.stdinStream = stream
				go waitStreamReply(stream.replySent, replyChan, stop)
			case corev1.StreamTypeStdout:
				sctx.stdoutStream = stream
				go waitStreamReply(stream.replySent, replyChan, stop)
			case corev1.StreamTypeStderr:
				sctx.stderrStream = stream
				go waitStreamReply(stream.replySent, replyChan, stop)
			case corev1.StreamTypeResize:
				sctx.resizeStream = stream
				go waitStreamReply(stream.replySent, replyChan, stop)
			default:
				err := fmt.Errorf("unexpected stream type: %q", streamType)
				logger := log.FromContext(ctx)
				logger.Error("unexpected stream type", err)
			}
		case <-replyChan:
			receivedStreams++
...
func (*v3ProtocolHandler) waitForStreams(ctx context.Context, streams <-chan streamAndReply, expectedStreams int, expired <-chan time.Time) (*streamContext, error) {
	sctx := &streamContext{}
	receivedStreams := 0
	replyChan := make(chan struct{})
	stop := make(chan struct{})
...
WaitForStreams:
	for {
		...
		case stream := <-streams:
			...
			switch streamType {
			case corev1.StreamTypeError:
				sctx.writeStatus = v1WriteStatusFunc(stream)
				go waitStreamReply(stream.replySent, replyChan, stop)
			case corev1.StreamTypeStdin:
				sctx.stdinStream = stream
				go waitStreamReply(stream.replySent, replyChan, stop)
...
func (*v2ProtocolHandler) waitForStreams(ctx context.Context, streams <-chan streamAndReply, expectedStreams int, expired <-chan time.Time) (*streamContext, error) {
	sctx := &streamContext{}
	receivedStreams := 0
	replyChan := make(chan struct{})
	stop := make(chan struct{})
...
func (*v1ProtocolHandler) waitForStreams(ctx context.Context, streams <-chan streamAndReply, expectedStreams int, expired <-chan time.Time) (*streamContext, error) {
	sctx := &streamContext{}
	receivedStreams := 0
	replyChan := make(chan struct{})
	stop := make(chan struct{})
...
WaitForStreams:
	...
	if sctx.stdinStream != nil {
		_ = sctx.stdinStream.Close()
	}
...
// v4WriteStatusFunc returns a WriteStatusFunc that marshals a given corev1 Status

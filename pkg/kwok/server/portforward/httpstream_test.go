...
func TestGetStreamPair(t *testing.T) {
	...
	dataStream.headers.Set(corev1.StreamType, corev1.StreamTypeData)
	...
	errorStream.headers.Set(corev1.StreamType, corev1.StreamTypeError)
...
func TestRequestID(t *testing.T) {
	...
	s := newFakeHTTPStream()
	s.headers.Set(corev1.StreamType, corev1.StreamTypeError)
	s.id = 1
	if e, a := "1", h.requestID(s); e != a {
		t.Errorf("expected %q, got %q", e, a)
	}

	s.headers.Set(corev1.StreamType, corev1.StreamTypeData)
	s.id = 3
	if e, a := "1", h.requestID(s); e != a {
		t.Errorf("expected %q, got %q", e, a)
	}
	...
	s.id = 7
	s.headers.Set(corev1.PortForwardRequestIDHeader, "2")

...
func handleHTTPStreams(ctx context.Context, req *http.Request, w http.ResponseWriter, portForwarder PortForwarder, podName, podNamespace string, uid types.UID, supportedPortForwardProtocols []string, idleTimeout, streamCreationTimeout time.Duration) error {
	...
	logger.Debug("Connection setting port forwarding streaming connection idle timeout", "connection", conn, "idleTimeout", idleTimeout)
...
func (h *httpStreamHandler) getStreamPair(requestID string) (*httpStreamPair, bool) {
	...
	if p, ok := h.streamPairs[requestID]; ok {
		h.logger.Debug("Connection request found existing stream pair", "connection", h.conn, "request", requestID)
		return p, false
	}

	h.logger.Debug("Connection request creating new stream pair", "connection", h.conn, "request", requestID)
...
func (h *httpStreamHandler) monitorStreamPair(ctx context.Context, p *httpStreamPair, timeout <-chan time.Time) {
	...
	case <-p.complete:
		h.logger.Debug("Connection request successfully received error and data streams", "connection", h.conn, "request", p.requestID)
	}
...
func (h *httpStreamHandler) requestID(stream httpstream.Stream) string {
	...
	if len(requestID) == 0 {
		h.logger.Debug("Connection stream received without requestID header", "connection", h.conn)
		...
		h.logger.Debug("Connection automatically assigning request ID from stream type and stream ID", "connection", h.conn, "request", requestID, "streamType", streamType, "stream", stream.Identifier())
	}
	return requestID
}
...
func (h *httpStreamHandler) run(ctx context.Context) {
	h.logger.Debug("Connection waiting for port forward streams", "connection", h.conn)
Loop:
	for {
		select {
		case <-h.conn.CloseChan():
			h.logger.Debug("Connection upgraded connection closed", "connection", h.conn)
			break Loop
		case stream := <-h.streamChan:
			requestID := h.requestID(stream)
			streamType := stream.Headers().Get(corev1.StreamType)
			h.logger.Debug("Connection request received new type of stream", "connection", h.conn, "request", requestID, "streamType", streamType)

			p, created := h.getStreamPair(requestID)
			if created {
				go h.monitorStreamPair(ctx, p, time.After(h.streamCreationTimeout))
			}
			if complete, err := p.add(stream); err != nil {
				err := fmt.Errorf("error processing stream for request %s: %w", requestID, err)
				logger := log.FromContext(ctx)
				logger.Error("add stream", err)
				p.printError(err.Error())
			} else if complete {

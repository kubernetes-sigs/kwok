...
func Convert_v1alpha1_StageSpec_To_internalversion_StageSpec(in *v1alpha1.StageSpec, out *StageSpec, s conversion.Scope) error {
	if len(in.Steps) == 0 && in.Next != nil {
		if in.Next.Event != nil {
			in.Steps = append(in.Steps, v1alpha1.StageStep{
				Event: in.Next.Event,
			})
		}

		if in.Next.Finalizers != nil {
			in.Steps = append(in.Steps, v1alpha1.StageStep{
				Finalizers: in.Next.Finalizers,
			})
		}

		if len(in.Next.Patches) != 0 {
			for _, patch := range in.Next.Patches {
				in.Steps = append(in.Steps, v1alpha1.StageStep{
					Patch: &patch,
				})
			}
		} else if in.Next.StatusTemplate != "" {
			template := in.Next.StatusTemplate

			subresource := "status"
			if in.Next.StatusSubresource != nil {
				subresource = *in.Next.StatusSubresource
			}

			var impersonation *v1alpha1.ImpersonationConfig
			if in.Next.StatusPatchAs != nil {
				impersonation = &v1alpha1.ImpersonationConfig{
					Username: in.Next.StatusPatchAs.Username,
				}
			}

			in.Steps = append(in.Steps, v1alpha1.StageStep{
				Patch: &v1alpha1.StagePatch{
...
func Convert_internalversion_StageSpec_To_v1alpha1_StageSpec(in *StageSpec, out *v1alpha1.StageSpec, s conversion.Scope) error {
	err := autoConvert_internalversion_StageSpec_To_v1alpha1_StageSpec(in, out, s)
	if err != nil {
		return err
	}

	if out.Next == nil {
		out.Next = &v1alpha1.StageNext{}
		for _, step := range out.Steps {
			if step.Event != nil {
				out.Next.Event = step.Event
			}

			if step.Finalizers != nil {
				out.Next.Finalizers = step.Finalizers
			}

			if step.Patch != nil &&
				step.Patch.Subresource == "status" &&
				step.Patch.Root == "status" &&
				step.Patch.Template != "" {
				out.Next.StatusTemplate = step.Patch.Template

				if step.Patch.Impersonation != nil {
					out.Next.StatusPatchAs = &v1alpha1.ImpersonationConfig{
						Username: step.Patch.Impersonation.Username,
					}
				}
			}

			if step.Delete {
				out.Next.Delete = step.Delete
			}
		}
	}
	return nil
...
// Convert_v1alpha1_MatchExpression_To_internalversion_MatchExpression converts a v1alpha1.MatchExpression to an internal version MatchExpression.
func Convert_v1alpha1_MatchExpression_To_internalversion_MatchExpression(in *v1alpha1.MatchExpression, out *MatchExpression, s conversion.Scope) error {
	if in.JQ == nil && in.Key != "" {
	...
	return autoConvert_v1alpha1_MatchExpression_To_internalversion_MatchExpression(in, out, s)
}

// Convert_v1alpha1_ExpressionFrom_To_internalversion_ExpressionFrom converts a v1alpha1.ExpressionFrom to an internal version ExpressionFrom.
func Convert_v1alpha1_ExpressionFrom_To_internalversion_ExpressionFrom(in *v1alpha1.ExpressionFrom, out *ExpressionFrom, s conversion.Scope) error {
	if in.JQ == nil && in.ExpressionFrom != "" {
		in.JQ = &v1alpha1.ExpressionJQ{
			Expression: in.ExpressionFrom,
		}
	}
	return autoConvert_v1alpha1_ExpressionFrom_To_internalversion_ExpressionFrom(in, out, s)
}

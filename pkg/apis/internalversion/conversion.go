...
func Convert_v1alpha1_KwokctlConfigurationOptions_To_internalversion_KwokctlConfigurationOptions(in *configv1alpha1.KwokctlConfigurationOptions, out *KwokctlConfigurationOptions, s conversion.Scope) error {
	...
	if format.ElemOrDefault(in.DisableKubeControllerManager) {
		disable = append(disable, consts.ComponentKubeControllerManager)
	}
	if format.ElemOrDefault(in.DisableKubeScheduler) {
		disable = append(disable, consts.ComponentKubeScheduler)
	}
	if format.ElemOrDefault(in.EnableMetricsServer) {
		enable = append(enable, consts.ComponentMetricsServer)
	}
	if in.KubeApiserverInsecurePort != 0 {
	...
	if !hasEtcd {
		return fmt.Errorf("etcd must be enabled")
	}
	...
	if !hasApiserver {
		return fmt.Errorf("kube-apiserver must be enabled")
	}

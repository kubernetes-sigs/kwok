...
`)
	...
	cmd.Flags().StringArrayVar(&flags.ComponentsParam, "component-param", flags.ComponentsParam, "Set parameters for the cluster, format: --param=.key1=value1 --param=.key2=value2")
...
func runE(ctx context.Context, flags *flagpole) error {
	...
	if exist {
		...
		for _, componentName := range flags.EnableComponents {
			...
			params := slices.FilterAndMap(flags.ComponentsParam, func(s string) (string, bool) {

...
//go:embed dashboard_deployment.yaml.tpl
var dashboardDeploymentYamlTpl string

var dashboardDeploymentYamlTemplate = template.Must(template.New("dashboard_deployment").Parse(dashboardDeploymentYamlTpl))
...
func BuildDashboardDeployment(conf BuildDashboardDeploymentConfig) (string, error) {
	...
	err = dashboardDeploymentYamlTemplate.Execute(buf, conf)
	if err != nil {
		return "", fmt.Errorf("failed to execute dashboard deployment yaml template: %w", err)
	}
	return buf.String(), nil
}

// BuildDashboardDeploymentConfig is the configuration for building the dashboard deployment
type BuildDashboardDeploymentConfig struct {

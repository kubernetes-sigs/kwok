...
func (c *Cluster) addKueueviz(ctx context.Context, env *env) (err error) {
	...
	err = c.ensureImage(ctx, conf.KueuevizBackendImage)
	if err != nil {
		return err
	}

	err = c.ensureImage(ctx, conf.KueuevizFrontendImage)
	if err != nil {
		return err
	}
...
func (c *Cluster) addJobSet(ctx context.Context, env *env) (err error) {
	...
	err = c.ensureImage(ctx, conf.JobSetImage)
	if err != nil {
		return err
	}
...
func (c *Cluster) addLWS(ctx context.Context, env *env) (err error) {
	...
	// Configure the lws
	err = c.ensureImage(ctx, conf.LWSImage)
	if err != nil {
		return err
	}
...
func (c *Cluster) addDescheduler(ctx context.Context, env *env) (err error) {
	...
	err = c.ensureImage(ctx, conf.DeschedulerImage)
	if err != nil {
		return err
	}
...
func (c *Cluster) ensureImage(ctx context.Context, image string) error {
	if c.isAppleContainer {
		return c.EnsureImage(ctx, []string{c.runtime, "image"}, image)
	}

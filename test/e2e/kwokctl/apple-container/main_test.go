...
var (
	runtimeEnv     = consts.RuntimeTypeAppleContainer
	updateTestdata = false
	testEnv        env.Environment
	pwd            = os.Getenv("PWD")
	rootDir        = path.Join(pwd, "../../../..")
	logsDir        = path.Join(rootDir, "logs")
	clusterName    = envconf.RandomName("kwok-e2e-apple-container", 24)
...
func TestMain(m *testing.M) {
	...
	testEnv.Setup(
		helper.BuildKwokImage(rootDir, testImage, consts.RuntimeTypeAppleContainer),

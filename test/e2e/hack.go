...
func CaseHack(kwokctlPath, clusterName, nodeName string) *features.FeatureBuilder {
	...
	return features.New("Hack Data").
		...
		Assess("Hack Data", func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			buf0 := &bytes.Buffer{}
			_, err := utilsexec.Command(utilsexec.WithWriteTo(ctx, buf0), kwokctlPath, "--name", clusterName, "kectl", "get", "no", nodeName, "-o", "yaml")
			if err != nil {
				t.Fatal(err)
			}
			if buf0.Len() == 0 {
			...
			buf1 := &bytes.Buffer{}
			_, err = utilsexec.Command(utilsexec.WithWriteTo(ctx, buf1), kwokctlPath, "--name", clusterName, "kectl", "delete", "no", nodeName)
			if err != nil {
				t.Fatal(err)
			}
			...
			_, err = utilsexec.Command(utilsexec.WithReadFrom(ctx, bytes.NewBuffer(buf0.Bytes())), kwokctlPath, "--name", clusterName, "kectl", "put", "no", nodeName, "--path", "-")
			if err != nil {
				t.Fatal(err)
			}

			buf2 := &bytes.Buffer{}
			_, err = utilsexec.Command(utilsexec.WithWriteTo(ctx, buf2), kwokctlPath, "--name", clusterName, "kectl", "get", "no", nodeName, "-o", "yaml")
			if err != nil {
				t.Fatal(err)
			}

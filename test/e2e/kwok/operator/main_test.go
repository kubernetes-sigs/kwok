...
// Package operator_test is a test environment for kwok.
...
var (
	...
	testOperatorImage = "localhost/operator:test"
)
...
func TestMain(m *testing.M) {
	...
	testEnv.Setup(
		...
		helper.BuildOperatorImage(rootDir, testOperatorImage, consts.RuntimeTypeDocker),
		envfuncs.CreateCluster(kind.NewProvider(), clusterName),
		helper.WaitForAllNodesReady(),
		envfuncs.LoadImageToCluster(clusterName, testImage),
		envfuncs.LoadImageToCluster(clusterName, testOperatorImage),

...
// CaseExecMapping creates a feature that tests exec mapping
func CaseExecMapping(nodeName, namespace string) *features.FeatureBuilder {
	...
	return features.New("Pod Exec Mapping").
		...
		Assess("test exec", func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			var ce = &v1alpha1.ClusterExec{
				ObjectMeta: metav1.ObjectMeta{
					Namespace: pods.Items[0].Namespace,
					Name:      pods.Items[0].Name,
				},
				Spec: v1alpha1.ClusterExecSpec{
					Execs: []v1alpha1.ExecTarget{
						{

...
function test_logs() {
  ...
  # Test basic scenario
  if ! result=$(kwokctl --name "${name}" kubectl -n "${namespace}" logs "${target}"); then
  ...
  # Test log tail
  if ! result=$(kwokctl --name "${name}" kubectl -n "${namespace}" logs --tail=2 "${target}"); then
  ...
  # Test log since time
  if ! result=$(kwokctl --name "${name}" kubectl -n "${namespace}" logs --since-time="2016-10-06T00:18:09.669794202Z" "${target}"); then
    echo "Error: logs tail failed"
    return 1
  fi
